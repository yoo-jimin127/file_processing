...
void createIndex(FILE *idxfp, FILE *recordfp) {
	...
	int totalRecord = 0, recordnum = 0;

	HeaderRecord headerRecord;
	DeletedRecord deletedRecord;
	Person p;

	readPage(recordfp, (char*)&headerRecord, 0);
	fseek(idxfp, 0, SEEK_SET);

	for (int i = 1; i < headerRecord.total_datapage; i++) {
		totalRecord = PAGE_SIZE / MAX_RECORD_SIZE; //총 레코드 수 구하기
		recordnum = totalRecord - ((totalRecord * (headerRecord.total_datapage - 1)) - headerRecord.total_record);
		readPage(recordfp, pagebuf, i); //해당 인덱스번째 페이지에 레코드 읽어오기

		for (int j = 0; j < totalRecord; j++) {
			memcpy(recordbuf, (j * MAX_RECORD_SIZE) + pagebuf, MAX_RECORD_SIZE);
			unpack(recordbuf, &p);

			memset(indexbuf, 0, 21);
			sprintf(indexbuf,"%s%d%d", p.id, i, recordnum); 
		}

		fseek(idxfp, 0, 21 * i);
		fwrite(indexbuf, 1, strlen(indexbuf), idxfp);
	}
}
...
void binarysearch(FILE *idxfp, const char *id, int *pageNum, int *recordNum) {
	...
	strcpy(keyvalue, id); //값 복사
	indexbuf = (char *)malloc(sizeof(char) * filesize);
	fread(indexbuf, 1, filesize, idxfp); //인덱스 파일의 내용을 버퍼에 복사
	//printf("indexbuf : %s\n", indexbuf);

	while (low <= high) {
		memset(tmpbuf, 0, 14);
		mid = (low + high) / 2; //만약 12개의 인덱스레코드가 있으면 mid의 초기값은 6
		strncpy(tmpbuf, &indexbuf[low * 21], 13);
		//printf("tmpbuf : %s\n", tmpbuf);

		if (strcmp(tmpbuf, keyvalue) == 0) { //두 주민번호가 같을 경우
			printf("#reads: %d\n", totalRecord);
			printf("id=%s\n", p.id);
			printf("name=%s\n", p.name);
			printf("age=%s\n", p.age);
		...
		else if (strcmp(tmpbuf, keyvalue) > 0) { //tmpbuf의 값이 더 클 때
			high = mid - 1;
		}

		else if (strcmp(tmpbuf, keyvalue) < 0) { //tmpbuf의 값이 더 작을 때
			low = mid + 1;
		}
	}
}
...
int main(int argc, char *argv[]) {
	...
	Person p; //person구조체
	char keyvalue[14]; //binary search에서 받은 인자를 저장할 배열

	int pagenum[PAGE_SIZE] = { 0,};
	int recordnum[MAX_RECORD_SIZE] = { 0,};
	...
	if (strcmp(argv[1], "a") == 0) { //a옵션을 주어 레코드 추가
	...
	else if (strcmp(argv[1], "i") == 0) { //i옵션을 주어 인덱스 파일 생성
		if ((idxfp = fopen(argv[3], "a+")) == NULL) {
			fprintf(stderr, "%s fopen() a+ mode error.\n", argv[3]);
			exit(1);
		}
	...
	else if (strcmp(argv[1], "b") == 0) { //b옵션을 주어 이진검색
		...
		binarysearch(idxfp, keyvalue, pagenum, recordnum);
	}

...
int main (int argc, char* argv[]) {
	...
	if ((write_fptr = fopen(copy_filename, "a+")) == NULL) { //파일을 읽기/덧붙이기 모드로 open
	...
	while (feof(read_fptr) == 0) { //파일포인터가 end of file이 아닐 때까지 반복
		fread(buf, sizeof(char) * 10, MAX_SIZE, read_fptr); //10바이트 단위로 원본파일에서 데이터를 읽어옴(복사)
		fwrite(buf, sizeof(char) * 10, MAX_SIZE, write_fptr); //10바이트 단위로 복사본파일에 버퍼에 저장된 데이터를 붙여넣음(붙여넣기)
		memset(buf, 0, MAX_SIZE); //buf 내용 초기화
	}
